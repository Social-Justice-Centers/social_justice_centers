    ...
    useEffect(() => {
        const fetchShifts = async () => {
            try {
                ...
                const filtered = Array.isArray(data) ? data.filter((s: Shift) => s.type === 'reported') : [];
                setShifts(filtered);
            } catch (err) {
            ...
            <div className="max-w-3xl mx-auto pt-14">
                ...
                <div className="flex items-center justify-center mb-8">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl font-bold" style={{ color: BRAND_GREEN }}>משמרות שדווחו</h1>
                        <Clock size={28} style={{ color: BRAND_GREEN }} />
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6" style={{ border: `2px solid ${BRAND_GREEN}` }}>
                    {shifts.length === 0 ? (
                        <p className="text-center text-gray-400 py-10 font-bold text-lg">לא נמצאו משמרות שדווחו</p>
                    ) : (
